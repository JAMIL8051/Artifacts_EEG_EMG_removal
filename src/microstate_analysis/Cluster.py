...
def findMeanCorrelation(subjectWiseData, subjects):
    ...
    meanValue = meanTrainData.mean(axis = 1, keepdims = True)

    gfp, gfp2, gfp_peaks = ModifiedKmeans.findGfpPeaks(meanTrainData)
    ...
    while n_maps<Configuration.numberOfCluster():    
        maps, labels, gfp_peaks, gev, cv = ModifiedKmeans.kmeans(meanTrainData, gfp, gfp2, gfp_peaks, meanValue, 
                                                                 n_maps, n_runs = 50)
