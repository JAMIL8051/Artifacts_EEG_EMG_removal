...
#Global function for identification of EMG artifacts by power analysis in the 45-70Hz
#freuquency band.
def identifyArtifacts(raw):
	...
	primary_raw, AF7_raw, AF8_raw = raw.copy(), raw.copy(), raw.copy()
	primary_picks = ['AF7','AF8','FT7','FT8']
	primary_raw = primary_raw.pick(picks = primary_picks)
	primary_power_45_70Hz = finalBandPower(raw, channel = primary_picks, N=1024,fs=512)
	
	
	AF7_raw = AF7_raw.pick(picks = ['Fp1', 'AF3', 'F3', 'F5', 'F7'])
	AF7_power_45_70Hz = finalBandPower(raw, channel = ['Fp1', 'AF3', 'F3', 'F5', 'F7'], N=1024,fs=512)
	
	
	AF8_raw = AF8_raw.pick(picks = ['Fp2', 'AF4', 'F4', 'F6', 'F8'])
	AF8_power_45_70Hz = finalBandPower(AF8_raw, channel = ['Fp2', 'AF4', 'F4', 'F6', 'F8'], N=1024,fs=512)
	FT7_raw = raw.copy()
	FT7_picks = ['F7', 'F5', 'FC5', 'C5', 'T7'] 
	FT7_raw = FT7_raw.pick(picks = FT7_picks)
	FT7_power_45_70Hz = finalBandPower(raw, channel = FT7_picks, N=1024,fs=512)
	
	FT8_raw = raw.copy()
	FT8_picks = ['F8', 'F6', 'FC6', 'C6', 'T8'] 
	FT8_raw = FT8_raw.pick(picks = FT8_picks)
	FT8_power_45_70Hz = finalBandPower(raw, channel = FT8_picks, N=1024,fs=512)

	Fz_raw = raw.copy()
	Fz_picks = ['Fz'] 
	Fz_raw = raw.pick(picks = Fz_picks)
	Fz_power_45_70Hz = finalBandPower(raw, channel = Fz_picks, N=1024,fs=512)
