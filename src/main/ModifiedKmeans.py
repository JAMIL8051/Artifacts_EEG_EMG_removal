...
# Helper function to optimize Modified k-means algorithm
def findGfpPeaks(data, distance=10, n_std=3):
    meanValue = data.mean(axis = 1, keepdims = True)
    gfp = data.std(axis=1)
    ...
    return gfp, gfp2, peaks, meanValue


def modifiedKmeans(data, gfp, gfp2, gfp_peaks, meanValue, numMaps, n_runs= 10, maxerr=1e-6, maxiter=1000):
    ...
    for run in range(n_runs):
        ...
        while ( (np.abs((var0-var1)/var0) > maxerr) and (n_iter < maxiter) ):
            ...
            C = np.dot(V, maps.T)
            C /= (numChannels*np.outer(gfp_values, np.std(maps, axis=1)))
